...
if ($busqueda_x_target) {
	...
	$targetversionoriginal=$targetversion;
        $targetversion=preg_replace("/[\s]+/","y",$targetversion);
	$targetversion=trim(`/bin/echo $targetversion|/bin/sed s/"\."/"xxxx"/`); //cambiem els . per x per saltar-nos el filtre de caracters alfanumerics anti-cakers
	...
	$targetversion=trim(preg_replace("/[\s]+/","_",$targetversionoriginal));
	$targetversion=trim(`/bin/echo $targetversion|/bin/sed s/"xxxx"/"\."/`);
//deixem els parametres preparats per sabueso: per exemple target=half_life y targetversion=mod_clients
	
	$command="./sabueso.sh $SO $target $targetversion";
...
//si la busqueda no es per target es per nom
	...
	$command="./sabueso.sh . $nom nom"; //sabueso mirara si el 3r parametro="nom" para buscar por nombre de exploit
}
